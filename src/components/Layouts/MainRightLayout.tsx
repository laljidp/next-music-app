...
MainRightLayout.Left = function ({ children }: { children: React.ReactNode }) {
  return (
    <div className="w-[40%] flex flex-col gap-2 [height:calc(100vh-115px)] relative">
      {children}
    </div>
  );
};
...
MainRightLayout.Separator = function () {
  return (
    <div className="w-0.5 bg-violet-300 relative rounded-full [height:calc(100vh-115px)]">
...
MainRightLayout.Right = function ({ children }: { children: React.ReactNode }) {
  return (
    <div className="w-[60%] overflow-y-auto [height:calc(100vh-115px)] scrollbar-thin flex p-2 items-start justify-center">
      {children}
    </div>
  );
};
